...
window = tk.Tk()
window.title("  Attendance System with Face Recognition ")
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            ...
            # cursor.execute("create database manually_fill_attendance")
            connection = pymysql.connect(
                host='localhost', user='root', password='', db='manual_attendance')
            cursor = connection.cursor()
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = 'C:/Users/dell/PycharmProjects/Attendance/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\dell\PycharmProjects\Attendance\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                timeStamp = datetime.datetime.fromtimestamp(
                    ts).strftime('%H:%M:%S')
                Hour, Minute, Second = timeStamp.split(":")
                fileName = "Attendance/Automatic Attendance/" + Subject + ".csv"  # "_" + date + \
                ...
                try:
                    ...
                    #     host='localhost', user='root', password='')
                    # cursor = connection.cursor()
                    # cursor.execute("create database attendancedb")
                    connection = pymysql.connect(
                        host='localhost', user='root', password='', db='auto_attendance')
                    cursor = connection.cursor()
                except Exception as e:
                    print(e)
                sql = "CREATE TABLE " + DB_Table_name + """
                ...
                root.configure(background='grey80')
                cs = 'C:/Users/dell/PycharmProjects/Attendance/' + fileName
    ...
    Notifica = tk.Label(windo, text="Attendance filled Successfully",
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:\Users\dell\PycharmProjects\Attendance\Attendance\-------Check atttendance-------"')

    attf = tk.Button(windo,  text="Check Sheets", command=Attf,
                     # fg="white", bg="black",
                     # width=12, height=1, activebackground="white", font=('times', 14, ' bold ')
                     )
    ...
    def log_in():
        ...
        if username == 'hasana':
            if password == '1':
                ...
                cs = 'C:/Users/dell/PycharmProjects/Attendance/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
message = tk.Label(window, text=" Attendance System", bg="black", fg="white", width=45,
...
AP = tk.Button(window, text="Check Registered students", command=admin_panel, bg="black", fg="white",
               width=19, height=1, activebackground="white", font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, text="Take Images", command=take_img, bg="black", fg="white",
                    width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images", bg="black", fg="white", command=trainimg,
                     width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, text="Automatic Attendance", bg="black", fg="white", command=subjectchoose,
                width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill, bg="black", fg="white",
                        width=20, height=3, activebackground="white", font=('times', 15, ' bold '))
